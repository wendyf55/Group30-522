...
def save_interaction_plot(train_df, interaction_path):
    ...
    base = (
        ...
        .encode(
            ...
            y=alt.Y("Rings:Q", title="Number of Rings (Age Proxy)"),
            color=alt.Color(
                "Sex Category:N",
                title="Sex Category",
                scale=alt.Scale(
                    domain=["Male", "Female", "Infant"],
    ...
    lines = (
        base.transform_regression("Shell_weight", "Rings", groupby=["Sex Category"])
        .mark_line(strokeWidth=3)
        .encode(color=alt.Color("Sex Category:N", title="Sex Category", legend=None))
    )
    ...
    (base + lines).properties(
        ...
        width=550,
        height=400,
    ).configure_axis(labelFontSize=12, titleFontSize=13).configure_legend(
