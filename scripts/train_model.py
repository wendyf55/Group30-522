...
def train_models(X, y, seed):
    ...
    lr_pipeline = make_pipeline(
        create_preprocessor(NUMERIC_FEATURES, CATEGORICAL_FEATURES), LinearRegression()
    )
    lr_pipeline.fit(X, y)
...
def evaluate_models(models, X, y):
    ...
    for name, model in models.items():
        ...
        results.append(
            {"Model": name, "RMSE": round(rmse, 4), "R2_Score": round(r2, 4)}
        )
...
def create_predictions_df(models, X, y):
    ...
    for name, model in models.items():
        ...
        for actual, predicted in zip(y, y_pred):
            predictions.append(
                {"Model": name, "Actual": actual, "Predicted": predicted}
            )
...
def create_actual_vs_predicted_chart(predictions_df):
    ...
    chart = (
        ...
        .properties(title="Actual vs Predicted Rings by Model")
    )
    return chart
...
def create_model_comparison_chart(metrics_df):
    ...
    chart = (
        ...
        .properties(width=200, height=300, title="Model Performance Comparison")
    )
    return chart
...
def create_residuals_chart(predictions_df):
    ...
    chart = (
        ...
        .properties(width=300, height=300)
        .facet(column=alt.Column("Model:N", title=None))
        .properties(title="Residuals Plot by Model")
    )
    return chart
...
def create_lr_scatter_chart(predictions_df, metrics_df):
    ...
    lr_predictions = predictions_df[
        predictions_df["Model"] == "Linear Regression"
    ].copy()
    ...
    # Create scatter plot
    scatter = (
        alt.Chart(lr_predictions)
        .mark_circle(opacity=0.5, color="steelblue", size=60)
        .encode(
            x=alt.X(
                "Actual:Q",
...
def train_and_save_results(train_path, test_path, output_prefix, seed):
    ...
    save_chart(create_residuals_chart(predictions_df), f"{output_prefix}_residuals.png")
    ...
    lr_coef_df = extract_lr_coefficients(
        models["Linear Regression"], NUMERIC_FEATURES, CATEGORICAL_FEATURES
    )
