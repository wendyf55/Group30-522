...
![Boxplot showing the distribution and outliers of target variable Rings.](../results/data_validation/target_boxplot.png){#dist_boxplot} 
...
![Histogram showing the distribution of target variable Rings.](../results/data_validation/target_histogram.png){#rings_hist}
...
![Correlation heatmap showing relationships between numerical features and the target variable (Rings).](../results/data_validation/correlation_plot.png){#rings_corr}
...
![EDA](../results/eda/pandas_profiling.html){#eda_prof}
...
![Number of rings (target variable) by shell weight (grams), grouped by Sex: Male (M), Female (F), and Infant (I). Linear regression lines for each group are shown.](../results/eda/interaction_plot.png){#interaction_plot}
...
![Actual Rings (x-axis) vs Predicted Rings (y-axis) for the linear regression model. The red dashed line represents perfect prediction (y = x). Points close to this line indicate accurate predictions, while deviations show prediction errors.](../results/model/model_results_actual_vs_predicted.png){#model_comp}
...
![Side-by-side comparison of RMSE (lower is better) and R² (higher is better) across all three models evaluated on the test set.](../results/model/model_results_model_comparison.png){#model_comp}
