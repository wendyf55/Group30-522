...
#### Correct Data File Format and Data Validation:

We used several validation tests to ensure data had the correct file format. We also completed data validation to ensure columns were of the expected data type, had no missing values, and were titled correctly. 
...
@fig-dist-boxplot shows that there are outliers on the high end (older abalones), but no anomalous values below 0 or unreasonably high values. The outliers represent legitimately old abalones rather than data errors.


![Histogram showing the distribution of target variable Rings.](../results/data_validation/target_histogram.png){#fig-rings-hist}

@fig-rings-hist shows that the distribution shows a slight right skew, with most abalones having between 7-12 rings. This suggests age distribution in the sample is not perfectly normal, which may affect model assumptions.
...
@fig-rings-corr shows that features show moderate positive correlations with Rings (0.5-0.6), indicating that physical measurements do contain predictive signal for age. However, there are high correlations among the features themselves (>0.9 between weights and dimensions), suggesting multicollinearity. This could affect linear regression coefficient interpretation, though it doesn't necessarily harm prediction accuracy. Non-linear models like Random Forest are less sensitive to multicollinearity. 
...
* There is high correlation between diameter, height, length and weights. We will explore non-linear models, which are less sensitive to multicollinearity.
...
# Load model metrics for inline references
metrics = pd.read_csv('../results/model/model_results_metrics.csv')

# Extract values for each model
lr_r2 = metrics.loc[metrics['Model'] == 'Linear Regression', 'R2_Score'].values[0]
lr_rmse = metrics.loc[metrics['Model'] == 'Linear Regression', 'RMSE'].values[0]

rf_r2 = metrics.loc[metrics['Model'] == 'Random Forest', 'R2_Score'].values[0]
rf_rmse = metrics.loc[metrics['Model'] == 'Random Forest', 'RMSE'].values[0]

svr_r2 = metrics.loc[metrics['Model'] == 'SVR (RBF Kernel)', 'R2_Score'].values[0]
svr_rmse = metrics.loc[metrics['Model'] == 'SVR (RBF Kernel)', 'RMSE'].values[0]

# Calculate variance improvement
variance_improvement = (svr_r2 - lr_r2) * 100
unexplained_variance = (1 - svr_r2) * 100
```
...
![Actual Rings (x-axis) vs Predicted Rings (y-axis) for the linear regression model. The red dashed line represents perfect prediction (y = x). Points close to this line indicate accurate predictions, while deviations show prediction errors.](../results/model/model_results_lr_scatter.png){#fig-lr-scatter}

@fig-lr-scatter shows that the model tends to under-predict for older abalones (high ring counts) and shows considerable scatter throughout, consistent with the moderate R² value of ~`{python} f"{lr_r2:.2f}"`. This suggests that linear relationships alone cannot fully capture how age relates to physical measurements.

```{python}
#| label: tbl-linear-coefs
...
coef_df = pd.read_csv('../results/model/model_results_lr_coefficients.csv')
display(coef_df)
```


@tbl-linear-coefs shows that since features are standardized, coefficient magnitudes are comparable. Whole weight shows the strongest positive association with age, while Shucked weight has a strong negative coefficient. This counterintuitive pattern (since both weights should increase with age) is likely due to multicollinearity among weight features—when correlated features are included together, their individual coefficients can be difficult to interpret. This further motivates our use of non-linear models that are less sensitive to multicollinearity.

### 8. Non-Linear Model Results

Given that the linear model explains only about `{python} f"{lr_r2*100:.0f}"`% of the variance (R² ≈ `{python} f"{lr_r2:.2f}"`), we now test non-linear models to see if they can capture more complex relationships in the data.
One additional motivation for trying these non-linear models is the strong multicollinearity we observed among several weight-related features. Linear regression can become unstable when predictors are highly correlated, because small changes in the data can lead to large swings in the estimated coefficients. Tree-based methods like Random Forest and kernel-based methods like SVR are much less sensitive to multicollinearity, so they provide a more robust way to leverage all correlated predictors without needing to manually drop or combine them.
...
@fig-model-comp shows that both non-linear models (Random Forest and SVR) outperform the linear baseline, with SVR achieving the best performance (lowest RMSE of `{python} f"{svr_rmse:.2f}"`, highest R² of `{python} f"{svr_r2:.2f}"`). This confirms that the relationship between physical measurements and abalone age contains non-linear components that simple linear regression cannot capture.

## Discussion

In this report, we tested three different models to find which model was best at predicting the number of rings (as a proxy for age) of abalone molluscs. The baseline **Linear Regression model** explains about `{python} f"{lr_r2*100:.0f}"`% of the variance in ring count (R² = `{python} f"{lr_r2:.2f}"`) using size and weight measurements, with a root mean squared error (RMSE) of approximately **`{python} f"{lr_rmse:.2f}"` rings**. This means the model's predictions are typically off by about `{python} f"{lr_rmse:.1f}"` rings on average.

The other two models tested were non-linear approaches. The **Random Forest model** achieves higher R² (`{python} f"{rf_r2:.2f}"`) and lower RMSE (`{python} f"{rf_rmse:.2f}"` rings) than the linear baseline, demonstrating that abalone growth patterns are not purely linear in relation to physical features. The **Support Vector Regression (SVR) with an RBF kernel** shows the best performance among the three models, with an RMSE of `{python} f"{svr_rmse:.2f}"` rings and R² of `{python} f"{svr_r2:.2f}"`. Both non-linear models capture approximately `{python} f"{variance_improvement:.0f}"`% more variance than the linear model, confirming that physical measurements and age have non-linear relationships.
